...
@Suppress("unused")
@Service
open class CommonFeatureToFeatureHistoryConsumer(@Qualifier("DbEventBus")val eventBus: EventBus,
                                                  val featureHistoryRepository: FeatureHistoryRepository) {
    ...
    internal fun handle(commonFeature : CommonFeature) {
        val featureHistory = toFeatureHistory(commonFeature)
        featureHistoryRepository.index(featureHistory)
    }

    internal fun toFeatureHistory(commonFeature: CommonFeature) : FeatureHistory {
        val result = scenarios(commonFeature.scenarios)
        return FeatureHistory(
                id = "${commonFeature.id}::${commonFeature.timestamp}",
